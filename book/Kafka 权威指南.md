...
* 了解系统的保证机制对于构建可靠的应用程序来说至关重要，这也是能够在不同条件下解释系统行为的前提，那么 Kafka 可以在哪些方面做出保证呢？ Kafka 可以保证分区消息的顺序；只有当消息被写入到分区的所有同步副本时（但不一定要写入磁盘），它才被认为是"已提交的"；只要还有一个副本是活跃的，那么已经提交的信息就不会丢失；消费者只能读取已经提交的消息。
