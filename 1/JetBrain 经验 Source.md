...
* 整理 IDEA 的细分使用教程：
    * Introduction to Version Control Systems in IntelliJ IDEA：① Git 的 commit message 框右上角的钟表标志可以显示最近的所有 commit message，然后选择一个开始做修改比较方便。② Git 的 commit and push 有快捷键，Mac 默认是 Option + Command + K。
* JetBrains APP 可能在粘贴 SQL 语句的时候出现问题，比如单引号出现了两次导致语法出错，全选重写粘贴一遍即可。
* JetBrains 挂代理：Preferences -> Appearance & Behavior -> System Settings -> Http Proxy: 选中 Manual proxy configuration -> HTTP，然后输入 Host 和 Port 分别是 0.0.0.0 和 1087。最后 Check connection：https://www.google.com 测试成功即可。
* IDEA 设置复制进来文件不自动格式化：Settings/Preferences | Editor | General | Smart Keys --> Reformat on Paste 设置为 None 即可。
* 设置了 IDEA 单文件查 History 的快捷键：Control + Command + H。
* 20190627    JetBrains 软件激活目前 key 的更新地址。
...
* 在 scratches 文件夹下建立分格式的文件夹是良好的工程实践，目前这个文件夹地址在：/Users/moqi/Library/Preferences/IntelliJIdea2019.1/scratches 目录，优势有两点：
    ...
    * 有关 Scratch 文件的更多介绍请看这里。
* IntelliJ IDEA中文乱码解决
* IDEA 文件对比文件：在一个文件上右键选择 Compare With（这里也可以使用快捷键 Command + D），然后 IDEA 会打开 Finder 选中第二个文件开始比较。也可以比较文件夹，链接有比较详细的说明。最后是打开 Diff 比较器出现两个文本比较框，非常实用，目前设置快捷键是 Control + Shift + Command + D。
* IDEA 提供直接载入 Github 项目功能，需要安装 Chrome 插件，然后 Git 项目旁边会有 IDEA 图标，点击即可（前提需要在本机配置好 SSH public key 到 Github 账户，如果出现克隆失败参考 Github 关于 SSH key gen 的说明即可）。
* IDEA 提供 SSH 保存功能：
...
* IDEA 激活码地址：记得先把这个两个地址放进 host：“0.0.0.0 account.jetbrains.com”及“0.0.0.0 www.jetbrains.com”
...
* 新建 Maven 项目时加入的参数：由于 maven 骨架和一些 jar 需要去 maven 的仓库下载，所以创建项目的时候速度会非常慢（外国的服务器你懂得），因此我们可以直接访问这里，把这个 xml 下载下来放到本地的 maven 目录中，然后在添加一个参数 archetypeCatalog = internal 就可以了。next 填写项目名称，finish 即可。
...
* IDEA 出现 "错误：找不到或无法加载主类"：解决方案：1. 如果原来或者刚才一直可用，突然出现这种情况，特别是对其他语言比如说 Scala 的类出现这种情况，则首先怀疑是文件夹并没有设置为 (Sources Root) 属性，如果在测试包下，则需要设定为 (Test Sources Root) 属性，目前发现在未操作的情况下 IDEA 可能会将其他语言的包设定为(Unmark as Sources Root) 属性。 2. 清空 IDEA 缓存: IDEA -> file -> invalidate Cache/restart 
